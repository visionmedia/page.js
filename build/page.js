...
;(function(){
  ...
  page.show = function(path, state, init, dispatch){
    ...
    if (!init) history.pushState(ctx.state, ctx.title, ctx.originalPath);
  };
  ...
  page.replace = function(path, state, init){
    ...
    ctx.init = init;
    page.dispatch(ctx);
    history.replaceState(ctx.state, ctx.title, ctx.originalPath);
  };
  ...
  function unhandled(ctx) {
    page.stop();
    window.location = ctx.originalPath;
  }
  ...
  Context.prototype.save = function(){
    history.replaceState(this.state, this.title, this.originalPath);
  };
