...
function isString(value) {
  return typeof value == 'string' ||
    (!isArray$1(value) && isObjectLike$2(value) && objectToString$2.call(value) == stringTag);
}

var lodash_size = size;
...
function concat() {
  ...
  return length
    ? arrayPush(isArray$2(array) ? copyArray(array) : [array], baseFlatten(args, 1))
    : [];
}
...
/** Built-in value references. */
var propertyIsEnumerable$2 = objectProto$4.propertyIsEnumerable;
...
function arrayLikeKeys$1(value, inherited) {
  // Safari 8.1 makes `arguments.callee` enumerable in strict mode.
  // Safari 9 makes `arguments.length` enumerable in strict mode.
  var result = (isArray$3(value) || isArguments$2(value))
...
var compact$2 = function compact(value) {
...
function createRouteTransitionMiddleware (routingBranch) {
  const routemap = lodash_last(routingBranch);
  const names = lodash_map(routingBranch, 'name');
  return (context, next) => {
...
function createUrlParsingMiddleware () {
    return (context, next) => {
        ...
        context.query = lib.parse(context.querystring);

        const parsedloc = location.search.split('?');
        const querystringloc = parsedloc[1] || '';
        context.locationquery = lib.parse(querystringloc);
...
class PageRouter {
	...
	registerRoute (routeBranch) {
		const path = '/' +
            lodash_compact(
                lodash_map(
                    routeBranch,
                    pmap => lodash_trim(pmap.path, '/')
                )
            ).join('/');

        const args = lodash_concat(
	...
	registerRouteBranch (routeBranch, routemap) {
		...
		if (lodash_size(routemap.children) > 0) {
			lodash_foreach(
				routemap.children,
